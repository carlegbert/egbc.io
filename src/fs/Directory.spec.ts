...
describe('Directory unit tests', function () {
  ...
  describe('#getChildrenByTypes()', function () {
    ...
    it('Finds directory children', function () {
      const dirs = testDir.getChildrenByTypes([FileKind.Directory])
      assert.equal(dirs.length, 1)
      assert.instanceOf(dirs[0], Directory)
    })

    it('Finds file children', function () {
      const files = testDir.getChildrenByTypes([FileKind.Text])
      assert.equal(files.length, 1)
      assert.instanceOf(files[0], TextFile)
    })

    it('Finds all children', function () {
      const children = testDir.getChildrenByTypes([FileKind.Directory, FileKind.Text])
    ...
    it('Returns empty array when directory is empty', function () {
      const dirs = emptyTestDir.getChildrenByTypes([FileKind.Text])
      const files = emptyTestDir.getChildrenByTypes([FileKind.Directory])
      const allTypes = emptyTestDir.getChildrenByTypes([FileKind.Directory, FileKind.Text])
